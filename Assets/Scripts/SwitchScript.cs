...
public class SwitchScript : MonoBehaviour
{
    ...
    void Update()
    {
        if (phase == 0) { anim.Play("SwitchActive"); }
        ...
        if (phase == 2 && stateInfo.IsName("SwitchBreak") && stateInfo.normalizedTime >= 1f) { phase = 3; }

        if (phase == 3) { anim.Play("SwitchInactive"); }
    }
}